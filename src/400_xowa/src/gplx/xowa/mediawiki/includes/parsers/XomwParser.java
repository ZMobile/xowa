...
import gplx.xowa.mediawiki.includes.parsers.doubleunders.Xomw_doubleunder_data;
import gplx.xowa.mediawiki.includes.parsers.doubleunders.Xomw_doubleunder_wkr;
import gplx.xowa.mediawiki.includes.parsers.headings.Xomw_heading_cbk__html;
import gplx.xowa.mediawiki.includes.parsers.headings.Xomw_heading_wkr;
import gplx.xowa.mediawiki.includes.parsers.hrs.Xomw_hr_wkr;
import gplx.xowa.mediawiki.includes.parsers.lnkes.Xomw_lnke_wkr;
import gplx.xowa.mediawiki.includes.parsers.lnkis.Xomw_lnki_wkr;
import gplx.xowa.mediawiki.includes.parsers.magiclinks.Xomw_magiclinks_wkr;
import gplx.xowa.mediawiki.includes.parsers.nbsps.Xomw_nbsp_wkr;
...
import gplx.xowa.mediawiki.includes.parsers.preprocessors.XomwPPNode_Hash_Array;
import gplx.xowa.mediawiki.includes.parsers.quotes.Xomw_quote_wkr;
import gplx.xowa.mediawiki.includes.parsers.tables.Xomw_table_wkr;
...
//		// These are variables reset at least once per parse regardless of $clearState
	...
	* @var Title
	*/
	public XomwTitleOld mTitle; // Title context, used for self-link rendering and similar things
...
//				XomwParser.EXT_LINK_URL_CLASS . '*)\p{Zs}*([^\]\\x00-\\x08\\x0a-\\x1F]*?)\]/Su';
	...
	public void Init_by_page(XomwTitleOld ttl) {
...
//		) {
	public void parse(XomwParserBfr pbfr, XomwParserCtx pctx,byte[] text, XomwTitleOld title, XomwParserOptions options) {this.parse(pbfr, pctx, text, title, options, true, true, -1);}
	public void parse(XomwParserBfr pbfr, XomwParserCtx pctx,
		byte[] text, XomwTitleOld title, XomwParserOptions options,
...
//				String relative = this.maybeDoSubpageLink(part1, subpage);
			...
			title = XomwTitleOld.newFromText(env, part1, ns);
...
//	}
	 ...
	 */
	public static XomwRevision statelessFetchRevision(XomwTitleOld title, XomwParser parser) { // parser = false
//		XomwRevision rev = XomwRevision.newKnownCurrent(wfGetDB(DB_REPLICA), title);
//
//		return rev;
		return null;
	}
...
//				rev = parser.fetchCurrentRevisionOfTitle(title);
				...
				rev = XomwRevision.newFromTitle(title);
			}
...
//		return $ret;
	 ...
	 * @return string
	 */
	public String extensionSubstitution(XophpArray params, XomwPPFrame frame) {
		return null;
