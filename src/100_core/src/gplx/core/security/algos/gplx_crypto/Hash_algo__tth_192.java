...
import gplx.core.security.algos.Hash_algo_w_prog;
public class Hash_algo__tth_192 implements Hash_algo, Hash_algo_w_prog {
	...
	void HashAllBytes(HashDlgWtr dialogWtr, IoStream stream, int leafCount) {
		...
		for (int i = 0; i < total; i++) {
			if (dialogWtr.Canceled()) dialogWtr.Fail(stream);
...
//					hashMain[j] = null;
	...
	byte[] CalcHash_leaf(byte[] raw, int i) {
		...
		byte[] rv = null;
		if (i == 0) {
			if (leaf0 == null || leaf0.length != raw.length + 1) {
				leaf0 = new byte[raw.length + 1];
			}
			rv = leaf0;
		}
		else {
			if (leaf1 == null || leaf1.length != raw.length + 1) {
				leaf1 = new byte[raw.length + 1];
			}
			rv = leaf1;
		}
	...
	public int CalcWorkUnits(long streamLength) {
		int leafCount = (int)(streamLength / blockSize);
...
class HashDlgWtr_ {
	public static HashDlgWtr Current = HashDlgWtrDefault.new_();
}
class HashDlgWtrDefault implements HashDlgWtr {
